# Luxundu-linux-project-work-in-progress-
a ubuntu linux based operating system with average ran usage of 400-490 mb and the known issues is that in live boot it shows ubuntu server and not working fastfetch


Download Link : https://mega.nz/file/8RYUEKwS#DTDZMq-xiGQuNO9Ku7DkHqA6aQxoXfSwJXdTfe9eMiU

Installation guide: download the ISO and flash it on to a USB drive using Rufus or balena etcher 
then boot into the live environment 
open the terminal and type : sudo subiquity
wait till setup loads
navigate through the menu using the arrow keys
select your language
and from this point it's like a normal install and don't forget about connecting to the internet (if needed)
